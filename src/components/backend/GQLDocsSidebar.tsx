...
function DocEntry({ doc, onUse, panelOpen }: DocEntryProps) {
    ...
    <div className="border-b border-border-subtle last:border-0 py-2.5">
      ...
      {open && (
        <div className="mt-2 space-y-3">
          <p className="text-[10px] text-text-muted leading-relaxed">{doc.description}</p>
                ...
                <div key={arg.name} className="bg-bg-base rounded-sm p-2 space-y-1 border border-border-subtle">
                  ...
                  {arg.values && arg.values.length > 0 && (
                    <div className="flex flex-wrap gap-1 pt-0.5">
                      {arg.values.map(v => (
                        <span key={v} className="text-[10px] font-mono bg-purple/10 text-purple-light px-1.5 py-0.5 rounded-sm border border-purple/20">{v}</span>
                      ))}
                    </div>
                  )}
                </div>
          ...
          <button
            ...
            tabIndex={panelOpen ? undefined : -1}
            className="text-[10px] px-2 py-1 rounded-sm bg-purple/15 text-purple-light hover:bg-purple/25 transition-colors cursor-pointer font-mono"
          >
            Use example
          </button>
        </div>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5">
          <div className="mb-6">
            ...
            {queries.map(doc => (
              <DocEntry key={doc.name} doc={doc} onUse={handleUse} panelOpen={open} />
            ))}
          </div>
